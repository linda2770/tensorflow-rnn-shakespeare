...
class CharRNN():
    def __init__(self, num_classes, batch_size=64, num_steps=50, cell_type,
    ...
    def rnn_layer(self):
        ...
        cell = cell_fn(self.rnn_size, reuse=tf.get_variable_scope().reuse)
        ...
        for _ in range(self.num_layers-1):
            cell = cell_fn(self.rnn_size, reuse=tf.get_variable_scope().reuse)                           
            cells.append(cell)
        if self.train_keep_prob > 0:
              cells = [tf.contrib.rnn.DropoutWrapper(cell,output_keep_prob=1-self.train_keep_prob) for cell in cells]  
